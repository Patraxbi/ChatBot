...
int scanPriority(char *input, int prio, ELEMENT **results, int *count)
{
    ...
    create_resultlist(input, keyList, nrElements, results, count);
...
{
    // Gets results for each priority
    ELEMENT *results[4]; int counters[4];
    *count = 0;
    
    for (int i = 0; i <= 3; i++)
    {
        scanPriority(input, i, &results[i], &counters[i]);
        *count += counters[i];
    }
    
    // Merges all results into a single list
    ELEMENT *total_results = (ELEMENT*) malloc((*count) * sizeof(ELEMENT));
    for (int i = 3, pos = 0; i >= 0; i--)
    {
        memcpy(total_results + pos, results[i], counters[i] * sizeof(ELEMENT));
        pos += counters[i];
    }
